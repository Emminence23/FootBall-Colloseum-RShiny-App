...
extract_data = function( Dir , league , link ){ 
  ...
  mainDir = paste( "./" , league , sep = "" , collapse = "" )
  season = 24 
  setwd( mainDir )
...
####################################################################################################################################
#
#Code to organize data into dat.Rdata

library(dplyr)

#setting working directory
setwd("C:/Users/patvv/OneDrive/Documents/mth-208-course-project-24group-1-main")

#declaring vectors for the seasons and the 5 leagues               
szn <- c(20, 21, 22, 23, 24)
leagues <- c("Bundesliga", "Premier_League", "Ligue_1", "Serie_A", "LaLiga")

 #declaring lists to store data            
combined_data <- vector("list", length = 5)
season_data <- vector("list", length = 5)

for (j in 1:5) {
  
  for (i in 1:5) {
    foo <- paste(leagues[i], szn[j], sep = "_")
    
    adv <- read.csv(paste("C:/Users/patvv/OneDrive/Documents/mth-208-course-project-24group-1-main/Data", leagues[i],
                          foo, "Player", "Advance.csv", sep = "/"))
    att <- read.csv(paste("C:/Users/patvv/OneDrive/Documents/mth-208-course-project-24group-1-main/Data", leagues[i],
                          foo, "Player", "Attack.csv", sep = "/"))
    def <- read.csv(paste("C:/Users/patvv/OneDrive/Documents/mth-208-course-project-24group-1-main/Data", leagues[i],
                          foo, "Player", "Defense.csv", sep = "/"))
    fp <- read.csv(paste("C:/Users/patvv/OneDrive/Documents/mth-208-course-project-24group-1-main/Data", leagues[i],
                         foo, "Player", "Fair_Play.csv", sep = "/"))
    gk <- read.csv(paste("C:/Users/patvv/OneDrive/Documents/mth-208-course-project-24group-1-main/Data", leagues[i],
                         foo, "Player", "Goalkeeping.csv", sep = "/"))
    
    adv <- adv %>% select(
      "Player", "Squad", "Nation", "Pos", "Pos2", "Age", "Total_Passes_Completed",
      "Total_Pass_Percentage", "Progression_Carries", "Key_Passes", "Passes_Into_Penalty_Area",
      "Touches", "Take_Ons_Attempted", "Take_Ons_Wins"
    )
    
    att <- att %>% select(
      "Player", "Goals.Assists", "Shots", "Shots_on_Target_Percentage", "Goals_per_Shot",
      "Goals_per_Shot_on_Target", "Shot_Creating_Actions", "Goal_Creating_Actions"
    )
    
    def <- def %>% select(
      "Player", "Tackles", "Tackles_Win_Percentage", "Challenges", "Challenges_Percentage",
      "Total_Blocks", "Interceptions", "Clearances", "Errors"
    )
    
    fp <- fp %>% select("Player", "Yellow_Card", "Red_Card", "Second_Yellow_Card", "Fouls")
    gk <- gk %>% select("Player", "Goals_Conceded", "Saves", "Saves_Percentage",
                        "Clean_Sheets_Percentage", "Penalty_Kicks_Save_Percentage")
    
    meow <- adv %>%
      full_join(att, by = "Player") %>%
      full_join(def, by = "Player") %>%
      full_join(fp, by = "Player") %>%
      full_join(gk, by = "Player")
    
    # Remove duplicates based on Player column
    meow <- meow[!duplicated(meow$Player), ]
    
    # Store the data frame in combined_data list
    combined_data[[i]] <- meow
  }
  
  # Combine all leagues data frames for the current season
  season_data[[j]] <- do.call(rbind, combined_data)
  
  # Add the season column
  season_data[[j]] <- season_data[[j]] %>% mutate(season = szn[j])
}

dat<- rbind(season_data[[1]], season_data[[2]], season_data[[3]],
            season_data[[4]], season_data[[5]])
